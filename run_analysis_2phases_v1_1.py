...
def main():
    ...
    Df = zapata['Df']
    h_zapata = zapata['h']
    z_top = -Df + h_zapata  # Tope de la zapata (ej: -0.5 + 0.4 = -0.1 m)
    z_base = -Df  # Base de la zapata (ej: -0.5 m)
    ...
    csv_fase1 = "settlements_fase1_gravedad_v1_1.csv"
    ...
    # -------------------------
    # V1.1: APLICAR PRESIÓN DISTRIBUIDA EN SUPERFICIE DE ZAPATA
    # -------------------------
    print("\n" + "="*80)
    print("V1.1: APLICANDO PRESIÓN DISTRIBUIDA EN SUPERFICIE DE ZAPATA")
    print("="*80)

    # Calcular presión uniforme a aplicar
    area_zapata = B_modelo * L_modelo
    presion_kPa = P_column / area_zapata  # kPa
    presion_Pa = presion_kPa * 1000.0  # Convertir a Pa (OpenSees trabaja en Pa)

    print(f"\nParámetros de carga:")
    print(f"  Carga total: {P_column:.2f} kN")
    print(f"  Área zapata (modelo 1/4): {area_zapata:.3f} m²")
    print(f"  Presión uniforme: {presion_kPa:.2f} kPa = {presion_Pa:.0f} Pa")

    # -------------------------
    # IDENTIFICAR CARAS SUPERIORES DE ZAPATA (Z = -0.1m)
    # -------------------------
    print("\nIdentificando caras superiores de elementos de zapata...")

    # Tolerancia para identificar superficie superior
    z_top_zapata = z_top  # -0.1 m
    tol_z = 0.05  # metros

    # Para cada elemento tetraédrico, las 4 caras triangulares son:
    # Face 1: nodos (1, 2, 3)
    # Face 2: nodos (1, 4, 2)
    # Face 3: nodos (2, 4, 3)
    # Face 4: nodos (3, 4, 1)

    # Lista para almacenar: (elemento_id, nodos de la cara)
    caras_superiores = []
    ...
    while cell_idx < len(cells):
        ...
        # Solo procesar elementos de zapata (concreto)
        if mat_id == mat_tag_zapata:
            # Coordenadas de los 4 nodos
            coords = [node_coords[n] for n in [n1, n2, n3, n4]]
            z_coords = [c[2] for c in coords]

            # Verificar cada una de las 4 caras triangulares
            caras_tetra = [
                (n1, n2, n3),  # Cara 1
                (n1, n4, n2),  # Cara 2
                (n2, n4, n3),  # Cara 3
            ...
            for cara in caras_tetra:
                # Obtener coordenadas Z de los 3 nodos de la cara
                z1 = node_coords[cara[0]][2]
                z2 = node_coords[cara[1]][2]
                z3 = node_coords[cara[2]][2]

                # Verificar si los 3 nodos están en la superficie superior
                en_superficie = (abs(z1 - z_top_zapata) < tol_z and
                               abs(z2 - z_top_zapata) < tol_z and
                               abs(z3 - z_top_zapata) < tol_z)

                if en_superficie:
                    # Verificar que no están en planos de simetría (X=0 o Y=0)
                    coords_cara = [node_coords[n] for n in cara]
                    x_coords = [c[0] for c in coords_cara]
                    y_coords = [c[1] for c in coords_cara]

                    en_plano_x0 = all(abs(x) < tol_z for x in x_coords)
                    en_plano_y0 = all(abs(y) < tol_z for y in y_coords)

                    # Solo agregar si NO está en plano de simetría
                    if not (en_plano_x0 or en_plano_y0):
                        caras_superiores.append((element_id, cara))
                        break  # Solo una cara por elemento puede estar en la superficie

        element_id += 1
        cell_idx += n_points + 1

    print(f"✓ Caras superiores identificadas: {len(caras_superiores)}")
    print(f"  (triángulos en Z ≈ {z_top_zapata:.2f} m)")

    # -------------------------
    # APLICAR PRESIÓN COMO FUERZAS NODALES EQUIVALENTES
    # -------------------------
    print(f"\nCalculando fuerzas nodales equivalentes a presión distribuida...")

    # FourNodeTetrahedron no soporta surfaceLoad, así que calculamos fuerzas equivalentes
    # Para cada cara triangular:
    #   Fuerza total = presión * área del triángulo
    #   Distribuir uniformemente en los 3 nodos

    # Acumular fuerzas por nodo
    fuerzas_nodos = {}  # nodo_id -> fuerza_z (negativa = hacia abajo)

    for ele_id, (n1, n2, n3) in caras_superiores:
        # Obtener coordenadas de los 3 nodos
        p1 = np.array(node_coords[n1])
        p2 = np.array(node_coords[n2])
        p3 = np.array(node_coords[n3])

        # Calcular área del triángulo usando producto cruzado
        v1 = p2 - p1
        v2 = p3 - p1
        area_triangulo = 0.5 * np.linalg.norm(np.cross(v1, v2))  # m²

        # Fuerza total en la cara (presión * área)
        fuerza_total_cara = presion_Pa * area_triangulo  # N

        # Distribuir uniformemente en los 3 nodos (negativa = hacia abajo)
        fuerza_por_nodo = -fuerza_total_cara / 3.0  # N

        # Acumular en cada nodo
        for nodo in [n1, n2, n3]:
            if nodo not in fuerzas_nodos:
                fuerzas_nodos[nodo] = 0.0
            fuerzas_nodos[nodo] += fuerza_por_nodo

    # Aplicar fuerzas acumuladas a los nodos
    for nodo_id, fuerza_z in fuerzas_nodos.items():
        ops.load(nodo_id, 0.0, 0.0, fuerza_z)

    # Verificar carga total aplicada
    carga_total_aplicada = -sum(fuerzas_nodos.values()) / 1000.0  # Convertir a kN

    print(f"✓ Presión distribuida aplicada en {len(caras_superiores)} caras triangulares")
    print(f"  Nodos cargados: {len(fuerzas_nodos)}")
    print(f"  Presión: {presion_kPa:.2f} kPa (hacia abajo)")
    print(f"  Carga total verificada: {carga_total_aplicada:.2f} kN (esperado: {P_column:.2f} kN)")
    print(f"  Error: {abs(carga_total_aplicada - P_column)/P_column * 100:.2f}%")

    # Calcular peso propio de zapata para presión total
    vol_zapata = B_modelo * L_modelo * h_zapata
    peso_zapata = vol_zapata * mat_zapata['rho'] * 9.81 / 1000.0  # kN
    presion_total_kPa = (P_column + peso_zapata) / area_zapata
    ...
    csv_fase2 = "settlements_fase2_incremental_v1_1.csv"
    ...
    if salida['generar_reporte']:
        ...
        with open(reporte_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"  Presión contacto: {presion_total_kPa:.2f} kPa\n")

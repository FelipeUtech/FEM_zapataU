...
# Configuración
RESULTADOS_DIR = 'resultados_Filtrado_Df_analisis'
OUTPUT_WORD = 'Analisis_Comparativo_Filtrado_Df.docx'
...
def crear_documento_word(datos, grafico_file):
    ...
    # Subtítulo
    subtitulo = doc.add_heading('FILTRADO Zn, Cu y Pb - Proyecto Porvenir', 2)
    ...
    p.add_run('FILTRADO Zn, Cu y Pb\n')
    p.add_run('Zapata: ').bold = True
    p.add_run('3.0m × 3.5m × 0.7m\n')
    p.add_run('Carga de columna: ').bold = True
    p.add_run('1,354.5 kN (total)\n')
    ...
    p = doc.add_paragraph(
        f'El análisis comparativo de diferentes profundidades de desplante (Df) para FILTRADO Zn, Cu y Pb '
